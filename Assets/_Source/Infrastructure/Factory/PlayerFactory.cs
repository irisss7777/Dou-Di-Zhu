...
namespace _Source.Infrastructure.Factory
{
    public class PlayerFactory : IDisposable, IInitializable
    {
        ...
        [Inject]
        private void Construct(GameLobbyModel gameLobbyModel)
        {
        ...
        private void CreatePlayer(string userId, string name, int maxPlayerCount)
        {
            ...
            var player = new PlayerModel(new PlayerData(userId, name, _gameLobbyModel), _playerAddedPublisher, _addCardPublisher, _selectCardPublisher, _deselectCardPublisher);
        }
