...
namespace _Source.Presentation.View.PlayerView
{
    public class PlayerView : MonoBehaviour, IPlayerView
    {
        ...
        public void Initialize(ISubscriber<PlayerSetCardCountDTO> playerSetCardCount, ISubscriber<PlayerPassedDTO> playerPass, ISubscriber<PlayerAddCardViewDTO> playerAddCard)
        {
            ...
            playerSetCardCount.Subscribe((message) => SetupCardCount(message)).AddTo(_disposable);
            playerPass.Subscribe((message) => DisplayPass(message.UserName, true)).AddTo(_disposable);
            playerAddCard.Subscribe((message) => DisplayPass(message.Name, false)).AddTo(_disposable);
        }
